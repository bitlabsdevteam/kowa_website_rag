---
name: dev
description: Pick the highest priority task from the current sprint and implement it with TDD, security scanning, and browser-based E2E testing. Use when implementing sprint tasks.
---
...
```
 ...
 └──────┬───────┘
        ▼
 ┌──────────────┐      ┌──────────────┐
 │ Write tests  │────▶│ Run tests     │──── PASS ──▶ Fix test
 │ FIRST (TDD)  │      │ (should fail) │   (test is wrong)
 └──────────────┘      └──────┬───────┘
                              │ FAIL (expected)
                              ▼
                       ┌──────────────┐
