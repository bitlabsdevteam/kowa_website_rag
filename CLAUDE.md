...
**Landing cinematic backdrop (`/` only).** A fixed Three.js scene (`components/hero-3d/hero-3d-scene.tsx`) paints a deep-green field `rgb(19,41,29)` with a drifting particle layer and a scroll-driven camera/parallax rig — no flying geometric objects. Every dark-glass box that floats over it (`.reference-site …`) derives from that **same** backdrop hue via the `--backdrop-base` / `--glass-panel` / `--glass-sub` / `--glass-raised` tokens, stepping up only in lightness per elevation, so backdrop and content boxes stay perfectly colour-matched. If you change the backdrop colour, change `--backdrop-base` **and** `PALETTE.backdrop` in the scene together, and re-derive the glass tokens from the same hue.
