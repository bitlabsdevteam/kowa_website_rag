...
import { RoundedPanel } from './primitives';
...
/** Final station: filled bags palletized beside a shipping container, with
 * the export route arcs drawing themselves in over the film's last beats
 * (scrubbed from the same progressRef the camera follows). */
export function ExportVignette({ progressRef }: { progressRef: ProgressRef }) {
  ...
  useFrame((state, delta) => {
    ...
    // Arcs draw in across the export/final beats (film progress 0.78 → 0.96).
    const reveal = MathUtils.clamp((progress - 0.78) / 0.18, 0, 1);
    easedRef.current = MathUtils.damp(easedRef.current, reveal, 4, delta);

    // Each arc draws independently: dot i of an arc appears once the eased
    // reveal passes its own fraction of that arc's run.
    let dotIndex = 0;
    ARCS.forEach((arc) => {
      for (let i = 0; i < arc.dots; i += 1) {
        const own = (i + 1) / arc.dots;
        const scale = MathUtils.clamp((easedRef.current - own) * arc.dots * 0.6 + 1, 0, 1);
    ...
    pulseRefs.current.forEach((pulse, index) => {
      if (!pulse) return;
      const lit = easedRef.current > 0.97 ? 1 : 0;
  ...
  return (
    <group name="export-vignette">
      {/* Loading platform. */}
      <RoundedPanel width={4.6} height={0.22} radius={0.05} color={FACTORY_PALETTE.silverLight} position={[STAGE_X.export + 0.7, 0.11, -0.1]} />
      {/* Shipping container with corrugation ridges and door seam. */}
      <RoundedPanel width={2.5} height={1.3} radius={0.05} color={FACTORY_PALETTE.accentStrong} position={[CONTAINER_X, 0.92, 0]} />
      {RIDGE_OFFSETS.map((offset) => (
        <mesh key={offset} position={[CONTAINER_X + offset, 0.92, 0.01]}>
          <planeGeometry args={[0.05, 1.14]} />
          <meshBasicMaterial color={FACTORY_PALETTE.white} transparent opacity={0.14} />
        </mesh>
      ))}
      <mesh position={[CONTAINER_X + 1.12, 0.92, 0.02]}>
      ...
      <mesh position={[STAGE_X.export - 1.35, 0.27, 0]}>
